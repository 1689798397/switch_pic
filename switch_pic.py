...
# 网络图片转化为磁盘图片
def net_disk(pic_path, pic_url):
    proxies = {"http": config["proxy"], "https": config["proxy"]}
    pic_data = requests.get(url=pic_url, proxies=proxies).content
    pic_name = str(uuid.uuid4()) + ".png"
    pic = pic_path + "/media/" + pic_name
    if not os.path.exists(pic_path + "/media"):
        os.makedirs(pic_path + "/media")
...
# base64转换为磁盘图片
def base64_disk(pic_path, base64str):
    base64str = base64str.replace("data:image/png;base64,", "")
    pic_name = str(uuid.uuid4()) + ".png"
    pic = pic_path + "/media/" + pic_name
    if not os.path.exists(pic_path + "/media"):
...
def read_md(md_absolute_path, target):
    ...
    with open(md_absolute_path, "r", encoding='UTF-8') as f:
        md_str = f.read()
        # 正则匹配所有图片标签
        pic_label_list = re.findall(r'!\[.*\]\(.*\)', md_str)
        # 遍历图片标签
